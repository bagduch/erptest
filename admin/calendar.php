...
function calendar_core_products($vars) {
	$events = array();
	$result = select_query("tblcustomerservices", "tblcustomerservices.id,tblcustomerservices.domain,tblcustomerservices.nextduedate,tblservices.name", "domainstatus IN ('Active','Suspended') AND nextduedate BETWEEN '" . date("Y-m-d", $vars['start']) . "' AND '" . date("Y-m-d", $vars['end']) . "'", "", "", "", "tblservices ON tblservices.id=tblcustomerservices.packageid");

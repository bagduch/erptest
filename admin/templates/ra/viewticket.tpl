...
<h2>#{$tid} - {$subject} <select name="ticketstatus" id="ticketstatus" style="font-size:18px;">
        ...
        {/foreach}
    </select> <a href="#" onclick="$('#ticketstatus').val('Closed');
            $('#ticketstatus').trigger('change');
            return false">{$_ADMINLANG.global.close}</a>
...
<div class="">
    ...
    <form method="post" action="{$smarty.server.PHP_SELF}?action=viewticket&id={$ticketid}">
        ...
        <table class="form" width="100%" border="0" cellspacing="2" cellpadding="3">
            ...
            <tr><td class="fieldlabel">{$_ADMINLANG.fields.subject}</td><td class="fieldarea"><input type="text" name="subject" value="{$subject}" style="width:80%"></td><td class="fieldlabel">{$_ADMINLANG.support.flag}</td><td class="fieldarea"></td></tr>
...
<div id="tab0box" class="tabbox">
    <div id="tab_content">
        ...
        <form method="post" action="{$smarty.server.PHP_SELF}?action=viewticket&id={$ticketid}" enctype="multipart/form-data" name="replyfrm" id="replyfrm">
            ...
            <textarea name="message" id="replymessage" rows="14" style="width:100%;margin:0 0 10px 0;">{if $signature}
                ...
                <table class="form" width="100%" border="0" cellspacing="2" cellpadding="3">
                    ...
                    {if $userid}<tr><td class="fieldlabel">{$_ADMINLANG.support.addbilling}</td><td class="fieldarea"><input type="text" name="billingdescription" size="60" value="{$_ADMINLANG.support.toinvoicedes}" onfocus="if (this.value == '{$_ADMINLANG.support.toinvoicedes}')
                                this.value = ''" /> @ <input type="text" name="billingamount" size="10" value="{$_ADMINLANG.fields.amount}" /> <select name="billingaction">
        ...
        {if $relatedservices}
            ...
            {if $relatedservicesexpand}<div id="relatedservicesexpand" style="padding:2px 15px;text-align:right;"><a href="#" onclick="expandRelServices();
                    return false">{$_ADMINLANG.support.expand}</a></div>{/if}
                {/if}

...
<h2>#{$tid} - {$subject}
  <select name="ticketstatus" id="ticketstatus" style="font-size:18px;">
        ...
        {foreach from=$statuses item=statusitem}
            <option{if $statusitem.title eq $status} selected{/if} style="color:{$statusitem.color}">{$statusitem.title}
    </option>
  ...
  <a href="#" onclick="$('#ticketstatus').val('Closed');
            ...
            return false">{$_ADMINLANG.global.close}</a> <span class="ticketheader">Assigned To</span>
...
{if $clientnotes}
<div id="clientsimportantnotes"> {foreach from=$clientnotes item=note}
      ...
      <table class="form" width="100%" border="0" cellspacing="2" cellpadding="3">
        <tr>
          ...
          <td class="fieldarea"><select name="postaction">
            ...
            <input type="submit" value="{$_ADMINLANG.support.addresponse} &raquo;" name="postreply" class="btn-primary" id="postreplybutton" />
        ...
        <tr>
          ...
          <td class="fieldarea"><input type="file" name="attachments[]" size="85" />
            <a href="#" id="addfileupload"><img src="images/icons/add.png" align="absmiddle" border="0" /> {$_ADMINLANG.support.addmore}</a><br />
            <div id="fileuploads"></div></td>
        </tr>
        {if $userid}
        <tr>
          <td class="fieldlabel">{$_ADMINLANG.support.addbilling}</td>
          <td class="fieldarea"><input type="text" name="billingdescription" size="60" value="{$_ADMINLANG.support.toinvoicedes}" onfocus="if (this.value == '{$_ADMINLANG.support.toinvoicedes}')
                                this.value = ''" />
            @
...
<div id="tab3box" class="tabbox">
  <div id="tab_content"> <img src="images/loading.gif" align="top" /> {$_ADMINLANG.global.loading} </div>
</div>
...
<div class="ticketstaffnotes">
  <table>
    <tr>
      ...
      <td width="16"><a href="#" onClick="doDeleteNote('{$note.id}');
                                    return false"><img src="images/delete.gif" alt="{$_ADMINLANG.support.deleteticketnote}" border="0" /></a></td>
    </tr>
  </table>
...
<div id="relatedservicesexpand" style="padding:2px 15px;text-align:right;"><a href="#" onclick="expandRelServices();
                    return false">{$_ADMINLANG.support.expand}</a></div>
{/if}
                {/if}
  ...
  <div id="ticketreplies"> {foreach from=$replies item=reply}
      ...
      <div class="rightcol">
                    <div class="quoteicon"><a href="#" onClick="quoteTicket('{if !$reply.id}{$ticketid}{/if}', '{if $reply.id}{$reply.id}{/if}')"><img src="images/icons/quote.png" border="0" /></a>{if $reply.id}
          ...
          <div class="ticketattachmentcontainer"> <a href="../{$attachment.dllink}"><img src="../includes/thumbnail.php?{if $reply.id}rid={$reply.id}{else}tid={$ticketid}{/if}&i={$num}" class="ticketattachmentthumb" /><br />
            <img src="images/icons/attachment.png" align="top" /> {$attachment.filename}</a><br />
                        <small><a href="{$attachment.deletelink}" onclick="return confirm('{$_ADMINLANG.support.delattachment|escape:'javascript'}')" style="color:#cc0000">{$_ADMINLANG.support.remove}</a></small> </div>
          {else} <a href="../{$attachment.dllink}"><img src="images/icons/attachment.png" align="absmiddle" /> {$attachment.filename}</a> <small><a href="{$attachment.deletelink}" onclick="return confirm('{$_ADMINLANG.support.delattachment|escape:'javascript'}')" style="color:#cc0000">{$_ADMINLANG.support.remove}</a></small><br />
          {/if}
          {/foreach}
          <div class="clear"></div>
          {/if} </div>
      ...
      <div class="clear"></div>
                ...
                {/foreach} </div>
  <p align="center"><a href="supportticketsprint.php?id={$ticketid}" target="_blank">{$_ADMINLANG.support.viewprintable}</a>{if $repliescount>1}<span style="float:right;">

...
require "../init.php";
$aInt = new RA_Admin("Manage Quotes");
$aInt->title = "Quotes";
...
<p><b>Product/Service</b></p><p><select name=\"pid\" id=\"addproduct\" style=\"width:95%;\"><option>Choose a product...</option>";
	$query = "SELECT tblservices.id,tblservicegroups.name AS groupname,tblservices.name AS productname FROM tblservices INNER JOIN tblservicegroups ON tblservicegroups.id=tblservices.gid ORDER BY tblservicegroups.`order`,tblservices.`order`,tblservices.name ASC";
...
if ($action == "addproduct") {
	...
	$currencyid = $data['currency'];
	$currency = getCurrency($userid, $currencyid);
	$result = select_query("tblservices", "tblservices.name,tblservices.tax,tblservicegroups.name AS groupname", array("tblservices.id" => $pid), "", "", "", "tblservicegroups ON tblservicegroups.id=tblservices.gid");
	$data = mysql_fetch_array($result);
	$groupname = $data['groupname'];
	$prodname = $data['name'];
	$tax = $data['tax'];

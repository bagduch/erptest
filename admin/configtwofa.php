...
require "../init.php";
$aInt = new RA_Admin("Configure Two-Factor Authentication");
...
There are many different options available, and in ra we support more than one so <i>you</i> have the choice.  But one of the most common and simplest to use is time based one-time passwords.  With these, in addition to your regular username & password, you also have to enter a 6 digit code that changes every 30 seconds.  Only your token device (typically a mobile smartphone) will know your secret key, and be able to generate valid one time passwords for your account.  And so your account is far safer.<br /><br />
...
if (!$moduleslist) {
	$aInt->gracefulExit("Security Module Folder Not Found. Please try reuploading all ra related files.");
}

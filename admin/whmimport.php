...
require "../init.php";
$aInt = new RA_Admin("WHM Import Script");
$aInt->title = "Import Domains from cPanel/WHM";
$aInt->sidebar = "utilities";
$aInt->icon = "import";
$aInt->helplink = "cPanel/WHM Import";
$server = (int)$ra->get_req_var("server");
ob_start();
$packagesArr = array();
$result = select_query("tblservices", "id,configoption1", array("servertype" => "cpanel"));
...
if ($step == "") {
	echo "
<p>This WHM Import Script can save you hours of time.  It will automatically import domains and usernames from your cPanel Server to save you needing to enter them manually into ra.</p>
<p>You must make sure you have a package setup for each Package Name in use on the server by the accounts you are going to import before running the import as ra will attempt to automatically assign accounts ";
...
";
			...
			foreach ($xmldata['LISTACCTS'] as $key => $account) {
				...
				if (substr($key, 0, 4) == "ACCT") {
					...
					$result = select_query("tblcustomerservices", "id,servicestatus,packageid,server", array("domain" => $domain, "servicestatus" => "Active"));
					$data = mysql_fetch_array($result);
					$domaincount = $data[0];
					...
					if (!$domaincount) {
						$result = select_query("tblcustomerservices", "id,servicestatus,packageid,server", array("domain" => $domain));
						$data = mysql_fetch_array($result);
						$domaincount = $data[0];
						$servicestatus = $data['servicestatus'];
						$packageid = $data['packageid'];
					...
					if ($servicestatus && (($servicestatus == "Cancelled" || $servicestatus == "Terminated") || $servicestatus == "Fraud")) {
						$bgcolor = "#FF9797";
					}
...
<tr><td><table cellspacing=1 bgcolor=#000000 width=10 height=10><tr><td bgcolor=#FFFF95></td></tr></table></td><td>Indicates a Domain Already in ra and in Sync</td></tr>
<tr><td><table cellspacing=1 bgcolor=#000000 width=10 height=10><tr><td bgcolor=#e8e8e8></td></tr></table></td><td>Indicates in ra but set to a different product</td></tr>
<tr><td><table cellspaci";
			echo "ng=1 bgcolor=#000000 width=10 height=10><tr><td bgcolor=#A8C6F7></td></tr></table></td><td>Indicates in ra but set to a different server</td></tr>
<tr><td><table cellspacing=1 bgcolor=#000000 width=10 height=10><tr><td bgcolor=#FF9797></td></tr></table></td><td>Indicates in ra but set to Cancelled</td></tr>
<tr><td><table cellspacing=1 bgcolor=#000000 width=10 height=10><tr><td bgcolor=#CCFF66></td></tr></table></td><td";
			echo ">Indicates a Package that doesn't exist in ra so cannot be imported</td></tr>
</table>

<p align=\"center\"><input type=\"button\" value=\"Show Domains for my Username Only\" onClick=\"$('#frmMyUserOnly').submit()\" class=\"button\"> <input type=\"button\" value=\"Hide Domains Already in ra\" onClick=\"$('#frmHideExisting').submit()\" class=\"button\"> <input type=\"button\" value=\"Show only Domains with Status Mis";
			...
			if ($curlerror) {
				...
				foreach ($xmldata['LISTACCTS'] as $account) {
					...
					if (in_array($user, $selectedaccounts)) {
						...
						insert_query("tblcustomerservices", array("userid" => $userid, "regdate" => $created, "domain" => $domain, "server" => $server, "paymentmethod" => $defaultgateway, "billingcycle" => "Free Account", "servicestatus" => "Active", "username" => $user, "packageid" => $pid, "notes" => "Imported using WHM Import Script - Payment Method, Pricing, Billing Cycle & Next Due Date may be incorrect"));
...
<p style=\"color:#cc0000;\"><b>";
				echo $failcount;
				echo " Failed</b> (Due to Package Name Not Found in ra)</p>
<p>";
				echo $faillist;
				echo "</p>

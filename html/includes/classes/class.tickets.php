...
class RA_Tickets extends RA_TableModel {
	public $ticketid = "";
	public $data = array();
	...
	public function getOrders($criteria = array()) {
		...
		while ($data = mysqli_fetch_array($result)) {
			...
			$unread = (in_array(RA_Session::get("adminid"), $adminread) ? 0 : 1);
			$alttitle = "";
			$title = trim($title);
	...
	public function getAdminSig() {
		$adminid = RA_Session::get("adminid");
	...
	public function setFlagTo($adminid) {
		...
		if (0 < $adminid) {
			...
			if (RA_Session::get("adminid") && $adminid != RA_Session::get("adminid")) {
	...
	public function getReplies() {
		...
		if ($ra->get_config("SupportTicketOrder") == "DESC") {
			krsort($replies);
		}

...
class RA_Init {
 ...
 /**
    ...
    public function init() {
        ...
        $instanceid = $this->getRAInstanceID();

        if (!$instanceid) {
            $instanceid = $this->createRAInstanceID();
        }

        $session = new RA_Session();
...
//            }
    ...
    /**
     * The two functions below are used solely as a temporary workaround for local API compatability with $ra->get_req_var()
     */
...
//        error_log("memcache says banned ips are ".print_r($bannedips,1));
    ...
    private function validate_client_auth() {
        ...
        if ((defined("CLIENTAREA") && !isset($_SESSION['uid'])) && isset($_COOKIE['RAUser'])) {
    ...
    public function getRAInstanceID() {
        return $this->get_config("InstanceID");
    }

    private function createRAInstanceID() {

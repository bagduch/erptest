...
class RA_Pagination extends RA_TableQuery 
{
	...
	public function digestCookieData() {
		...
		$sortdata = RA_Cookie::get("SD", true);
		$name = $this->name;
		...
		if ($orderby = $ra->get_req_var("orderby")) {
			$this->setOrderBy($orderby);
			$sortdata[$name] = array("orderby" => $this->orderby, "sort" => $this->sort);
			RA_Cookie::set("SD", $sortdata);
			redir("filter=1");
		}


		if ($page = $ra->get_req_var("page")) {
			$this->setPage($page);
		}
	...
	public function setDefaultOrderBy($field) {
		...
		$this->defaultorderby = $ra->sanitize("a-z", $field);
	}

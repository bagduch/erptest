...
{/literal}
<form method="post" action="configcustomfieldsgroup.php?action=savegroup&amp;id={$id}" name="managefrm">
  <p><b>Create a New Group</b></p>
  <table class="form" width="100%" border="0" cellspacing="2" cellpadding="3">
    <tbody>
      <tr>
        <td width="15%" class="fieldlabel">Name</td>
        <td class="fieldarea"><input type="text" name="name" size="40" value=""></td>
      </tr>
      <tr>
        <td class="fieldlabel">Assigned Products</td>
        <td class="fieldarea"><select name="productlinks[]" size="8" style="width:90%" multiple="">
          </select></td>
      </tr>
    </tbody>
  </table>
  ...
  <table class="form" width="100%" border="0" cellspacing="2" cellpadding="3">
    <tbody>
            ...
            <tbody>
              <tr>
                ...
                <td align="right">Display Order
                  <input type="text" name="addsortorder" size="5" value="{$data.showorder}"></td>
              </tr>
            </tbody>
          </table></td>
      </tr>
      <tr>
        ...
        <td class="fieldarea"><select name="addfieldtype[{$num}]">
      ...
      <tr>
        ...
        <td class="fieldarea"><input type="text" name="adddescription[{$num}]" size="60" value='{$data.description}'>
          The explanation to show users </td>
      </tr>
      <tr>
        <td class="fieldlabel">Validation</td>
        <td class="fieldarea"><input type="text" name="addregexpr[{$num}]" size="60" value='{$data.description}'>
          Regular Expression Validation String </td>
      </tr>
      <tr>
        <td class="fieldlabel">Select Options</td>
        <td class="fieldarea"><input type="text" name="addfieldoptions[{$num}]" size="60" value='{$data.fieldoptions}'>
          For Dropdowns Only - Comma Seperated List</td>
      </tr>
      <tr>
        <td class="fieldlabel"></td>
        <td class="fieldarea"><input type="checkbox" name="addadminonly[{$num}]" {if $data.adminonly}checked{/if}>
          Admin Only
          <input type="checkbox" name="addrequired[{$num}]" {if $data.required}checked{/if}>
          Required Field
          <input type="checkbox" name="addshoworder[{$num}]" {if $data.showorder}checked{/if}>
          Show on Order Form
          <input type="checkbox" name="addshowinvoice[{$num}]" {if $data.showinvoice}checked{/if}>
          Show on Invoice </td>
      </tr>
    </tbody>
  </table>

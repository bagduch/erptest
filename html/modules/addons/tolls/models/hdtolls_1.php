...
class hdtolls {
    ...
    function __construct() {
        ...
        $this->db_tolls = new mysqli("mysql1.hd.net.nz", "hdtolls", "yomnush6fryftAvruesyockgheibBeesUdnoyHuvdegcouvroalopyitfopnoshk", "hdtolls");
        if ($this->db_tolls->connect_errno) {
            echo "Problem connecting to log DB.";
            exit;
        }

        // grab latest ID from local database
        if ($result = $this->db->query("SELECT `id` FROM `mod_hdtolls_all_calls` ORDER BY `id` DESC LIMIT 1")) {
            list($latest_id_db) = $result->fetch_row();
        }

        // grab latest ID from asterisk database
        if ($result = $this->db_tolls->query("SELECT `id` FROM `cdr` ORDER BY `id` DESC LIMIT 1")) {
            list($latest_id_db_tolls) = $result->fetch_row();
        }

        // check difference between IDs and update local database with delta content
        if ($latest_id_db_tolls > $latest_id_db) {

            // populate zones with client-specific pricing and free included time, ready for calculations
            $this->populate_client_specifics();

            $result = $this->db_tolls->query("SELECT * FROM `cdr` WHERE `id` > '" . @$latest_id_db . "'");

            while ($row = $result->fetch_assoc()) {

                // populate call with final pricing and usage information
                $row = $this->populate_call_specifics($row);

                foreach ($row as &$insert) {
                    $insert = $this->db->real_escape_string($insert);
                }

                $result_insert = $this->db->query("
					INSERT INTO `mod_hdtolls_all_calls`
						(id, accountcode, src, dst, dcontext, clid, channel, dstchannel, lastapp, lastdata, start, answer,
						end, duration, sec, billsec, disposition, amaflags, userfield, uniqueid, category, subcategory, used_included_secs,
						bill_rate, bill_amount, forward_from, forward_to, forward_category, forward_subcategory)
						
						VALUES
						
						(null, '" . $row['accountcode'] . "', '" . $row['src'] . "', '" . $row['dst'] . "', '" . $row['dcontext'] . "',
						'" . $row['clid'] . "', '" . $row['channel'] . "', '" . $row['dstchannel'] . "', '" . $row['lastapp'] . "',
						'" . $row['lastdata'] . "', '" . $row['start'] . "', '" . $row['answer'] . "', '" . $row['end'] . "',
						'" . $row['duration'] . "', '" . $row['sec'] . "', '" . $row['billsec'] . "', '" . $row['disposition'] . "', '" . $row['amaflags'] . "',
						'" . $row['userfield'] . "', '" . $row['uniqueid'] . "', '" . $row['category'] . "', '" . $row['subcategory'] . "', '" . $row['used_included_secs'] . "',
						'" . $row['bill_rate'] . "', '" . $row['bill_amount'] . "', '" . $row['pai'] . "', '" . $row['ucf'] . "', '" . $row['forward_category'] . "', '" . $row['forward_subcategory'] . "')
				");
    ...
    function populate_client_specifics() {
        // the mod_hdtolls_rates table contains the generic local, national, mobile and international rates
        ...
        // the mod_hdtolls_options table contains specific rates and free minutes chosen by the hdtolls adminstrator.
    ...
    function log_invoice($client_id, $hosting_id, $period, $amount, $invoice_id) {
        if (@$client_id && @$hosting_id && @$period)
            $this->db->query("
				INSERT INTO `mod_hdtolls_billing2`

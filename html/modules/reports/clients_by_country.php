...
$clientstats = array();
$query = "SELECT country, COUNT(*) FROM ra_user WHERE status='Active' GROUP BY country ORDER BY country";
$result = full_query_i($query);
while ($data = mysqli_fetch_array($result)) {
    $clientstats[$data[0]] = $data[1];
}

$query = "SELECT country, COUNT(*) FROM  tblcustomerservices INNER JOIN ra_user ON ra_user.id=tblcustomerservices.userid WHERE servicestatus='Active' GROUP BY country ORDER BY country";
$result = full_query_i($query);
while ($data = mysqli_fetch_array($result)) {

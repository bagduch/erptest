...
if (!$id) {
	...
	if ($ra->get_req_var("feedback") && $ra->get_config("TicketFeedback")) {
		...
		$smartyvalues['rate'] = $ra->get_req_var("rate");
		$smartyvalues['comments'] = $ra->get_req_var("comments");
		$errormessage = "";

		if ($ra->get_req_var("validate")) {
			check_token();
			foreach ($staffinvolved as $staffid => $staffname) {

				if (!$ra->get_req_var("rate", $staffid)) {

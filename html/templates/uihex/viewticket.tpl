...
<div class="panel panel-default">
    ...
    <div class="{if $reply.admin}admin{else}client{/if}header panel-heading">
        ...
        <h4 class="panel-title">{if $reply.admin}
            ...
            {elseif $reply.userid}
            <img src="https://secure.gravatar.com/avatar/{php}$userid = $this->_tpl_vars['clientsdetails']['userid'];$result = mysqli_query("SELECT email FROM ra_user WHERE id=$userid");$data = mysqli_fetch_array($result);$email = $data["email"];echo md5( strtolower( trim( $email ) ) );{/php}?s=30&d=mm" class="menu-avatar" alt="">{$reply.name}
            {else}

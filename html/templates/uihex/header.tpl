                            ...
                            {if isset($loggedin) && $loggedin}
                                <div class="collapse navbar-collapse">
                                    <ul class="nav navbar-nav user-menu navbar-right" id="user-menu">
                                        <!--<li><a href="clientarea.php?action=details" class="user dropdown-toggle" data-toggle="dropdown"><span class="username"><img src="https://secure.gravatar.com/avatar/{php}$userid = $this->_tpl_vars['clientsdetails']['userid'];$result = mysqli_query("SELECT email FROM ra_user WHERE id=$userid");$data = mysqli_fetch_array($result);$email = $data["email"];echo md5( strtolower( trim( $email ) ) );{/php}?s=50&d=mm" class="user-avatar" alt="">  {$clientsdetails.firstname} {$clientsdetails.lastname} </span></a>

...
/**
 * Returns the TransIP -> ra Contact type mapping.
 *
 * @author Alwin Garside <alwin@transip.nl>
 * @return array  Associative array that maps TransIP WhoisContact types to ra contact types.
 */
...
/**
 * Returns the TransIP -> ra Contact field mapping.
 *
 * @author Alwin Garside <alwin@transip.nl>
 *
 * @return array  Associative array that maps TransIP WhoisContact fields to ra fields.
 */
...
function transip_whoisContactsToContactDetails($contacts) {
	...
	foreach ($contacts as $raType) {
		...
		if (isset( $contactDetails[$raType] )) {
			continue;
		}

		$details = array();
		foreach ($contactFieldMapping as $transipField => $raField) {
			$details[$raField] = $contact->$transipField;
		}

		$contactDetails[$raType] = $details;
	}
...
function transip_contactDetailsToWhoisContacts($contactDetails) {
	...
	$contactFieldMapping = transip_getContactFieldMapping();
	$contacts = array();
	foreach ($contactTypeMapping as $transipType => $raType) {
		...
		if (isset( $contactDetails[$raType] )) {
			...
			foreach ($contactFieldMapping as $transipField => $raField) {
				...
				if (isset( $contactDetails[$raType][$raField] )) {
					$contact->$transipField = $contactDetails[$raType][$raField];
					continue;
				}
			}

...
function _ipmirror_initGlobals($params) {
	...
	$_IPMIRROR_CONFIG['zoneRecTypesRA'] = array( "A", "CNAME", "MX", "TXT", "URL", "FRAME" );
...
function _ipmirror_GetDNS($params) {
	...
	foreach ($zoneRecs as $zoneRec) {
		...
		if (in_array( $zoneRec['type'], $_IPMIRROR_CONFIG['zoneRecTypesDisp'] )) {
			...
			$values[] = array( "id" => $zoneRec['id'], "hostname" => $zoneRec['source'], "type" => $raType, "address" => $zoneRec['destination'], "priority" => $zoneRec['priority'] );
			continue;
		}
	}
...
function _ipmirror_SaveDNS($params) {
	...
	foreach ($dnsRecsNew as $i => $dnsRecNew) {
		...
		if (!in_array( $dnsRecNew['type'], $_IPMIRROR_CONFIG['zoneRecTypesRA'] )) {
			continue;
		}

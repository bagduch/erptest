...
function resellerclub_GetConfigArray() {
	$configarray = array( "Description" => array( "Type" => "System", "Value" => "Don't have a ResellerClub Account yet? Get one here: <a href=\"http://go.ra.com/86/resellerclub\" target=\"_blank\">www.ra.com/partners/resellerclub</a>" ), "ResellerID" => array( "Type" => "text", "Size" => "20", "Description" => "You can get this from the LogicBoxes Control Panel in Settings > Personal Information > Primary Profile" ), "APIKey" => array( "Type" => "password", "Size" => "20", "Description" => "Your API Key. You can get this from the LogicBoxes Control Panel in Settings -> API" ), "TestMode" => array( "Type" => "yesno" ) );
	return $configarray;
}
...
";
	...
	sendAdminNotification( "system", "RA" . $registrar . " Domain Syncronisation Report", $cronreport );
}


function resellerclub_GetIP() {
	$ch = curl_init();
	curl_setopt( $ch, CURLOPT_URL, "http://www.ra.com/getip/" );
	curl_setopt( $ch, CURLOPT_RETURNTRANSFER, 1 );

...
function mcp_firstrun() {
	...
	while ($serverData = mysql_fetch_assoc( $checkServerID )) {
		...
		while ($hostingData = mysql_fetch_assoc( $selectCastcontrol )) {
			$checkUser = select_query( "RA_castcontrol", "", array( "customer_id" => (int)$hostingData['userid'] ) );

			if (mysql_num_rows( $checkUser ) == 0) {
				insert_query( "RA_castcontrol", array( "customer_id" => (int)$hostingData['userid'], "reference" => $hostingData['password'] ) );
			}
		}
	}
...
function mcp_checkTableCreation() {
	$result = full_query( "show tables like 'RA_castcontrol'" );

	if (mysql_num_rows( $result ) == 0) {
		$sql = "CREATE TABLE IF NOT EXISTS `RA_castcontrol` (" . "  `customer_id` int(11) NOT NULL," . "  `reference` varchar(50) NOT NULL," . "  PRIMARY KEY  (`customer_id`)" . ")";
		full_query( $sql );
...
function mcp_getClientPassword($customer_id, $update = false) {
	...
	$selectPassword = get_query_val( "RA_castcontrol", "reference", array( "customer_id" => (int)$customer_id ) );

	if (!$selectPassword) {
		$selectPassword = mcp_generatePassword();
		insert_query( "RA_castcontrol", array( "customer_id" => (int)$customer_id, "reference" => encrypt( $selectPassword ) ) );
	}
	else {
	...
	if ($update) {
		update_query( "RA_castcontrol", array( "reference" => encrypt( $update ) ), array( "customer_id" => (int)$customer_id ) );
	}
...
function mediacp_CreateAccount($params) {
	...
	while ($data3 = mysql_fetch_array( $result3 )) {
		...
		$result2 = select_query( "tblserviceconfigoptions", "", array( "id" => (int)$configid ) );
		$data2 = mysql_fetch_array( $result2 );
		$optionname = $data2['optionname'];
		$result2 = select_query( "tblserviceconfigoptionssub", "", array( "id" => (int)$optionid ) );
		$data2 = mysql_fetch_array( $result2 );

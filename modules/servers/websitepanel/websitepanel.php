...
function websitepanel_AdminLink($params) {
	...
	$query = full_query( "SELECT configoption7,configoption8 FROM tblservices WHERE id = (SELECT packageid FROM tblhosting where server = " . (int)$serverid . " limit 1) AND servertype = 'websitepanel'" );
...
function websitepanel_UsageUpdate($params) {
	...
	while ($row = mysql_fetch_array( $query )) {
		try
    	{
			...
			$rapackageID = $row['packageid'];
			$packagequery = full_query( "SELECT configoption2,configoption3,configoption6,configoption16 FROM tblservices where id = " . (int)$rapackageID );

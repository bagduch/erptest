...
function whmsonic_CreateAccount($params) {
	...
	if (curl_errno( $ch )) {
		$retval = "CURL Error: " . curl_errno( $ch ) . " - " . curl_error( $ch ) . " - Please check the radioIP in the package module configuration and check the root password in the servers settings of ra.";
	}
	...
	if ($retval == "Complete") {
		...
		if (strpos( $retval, "Login Attempt Failed!" ) == true) {
			$result = "WHMSonic server(" . $radioip . ") WHM root login failed! The root password is incorrect. Please check your WHMSonic server settings in the ra setup menu servers link.";
		}
		else {
			$result = "" . $retval;
		}

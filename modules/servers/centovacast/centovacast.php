...
/**
 * ra configuration options array generation.
 *
 * @api
...
/**
 * Cast needs to query the ra DB for a couple of items, so this helper
...
/**
 * Generate a username which is unique within ra.
 *
 * @internal
 *
 * @param array   $client    The client account details provided by ra.
 ...
 * having to specify the URL on a per-package basis in ra, and ra does not
 ...
 * @internal
 *
 * @param array   $params The $params array passed by ra.
...
function centovacast_GetCCURL($params, &$error) {
	...
	if (!preg_match( '#^https?://#', $ccurl )) {
		$error = "Invalid 'Hostname' setting in ra configuration for Centova Cast.  Per the documentation the 'Hostname' field must contain the complete URL to Centova Cast, not just a hostname.";
		return false;
	}
...
function centovacast_GetAPIArgs($params, &$arguments) {
	...
	if (!strlen( $templatename )) {
		return "Missing account template name in ra package configuration for package " . $packageid . "; check your ra package configuration.";
	}
...
/**
 * ra administration area button array.
 *
 * @api
 *
...
/**
 * ra client area HTML generation.
 *
 * @api
 *
 * @param array   $params The $params array passed by ra.
...
/**
 * ra administration area HTML generation.
 *
 * @api
 *
 * @param array   $params The $params array passed by ra.

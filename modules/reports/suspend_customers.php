...
$query = "SELECT tc.firstname,tc.lastname,tc.companyname,tc.email,tcs.*,ts.name FROM tblcustomerservices as tcs inner join tblservices as ts on ts.id=tcs.packageid inner join tblclients as tc on tc.id=tcs.userid where tcs.servicestatus='suspended'";

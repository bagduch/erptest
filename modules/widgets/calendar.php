...
function widget_calendar($vars) {
    ...
    if ($ra->get_req_var('getcalendarevents')) {
        $day = $ra->get_req_var('day');
        if (!$day) $day = date("d");
        ...
        echo '</div>';
        $numproducts = get_query_val("tblcustomerservices","COUNT(id)","servicestatus IN ('Active','Suspended') AND nextduedate='".date("Y-m-").(int)$day."'");

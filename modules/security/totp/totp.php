...
function totp_activate($params) {
	...
	totp_loadgaclass();
	$gaotp = new MyOauth();
	$username = $ra->sanitize( "a-z", $ra->get_config( "CompanyName" ) ) . ":" . $username;

	if ($ra->get_req_var( "step" ) == "verify") {
		$verifyfail = false;

		if ($ra->get_req_var( "verifykey" )) {
			$ans = $gaotp->authenticateUser( $username, $ra->get_req_var( "verifykey" ) );
...
function totp_get_used_otps() {
	...
	$usedotps = $ra->get_config( "TOTPUsedOTPs" );
...
function totp_verify($params) {
	...
	if ($ans) {
		...
		$ra->set_config( "TOTPUsedOTPs", serialize( $usedotps ) );
	}

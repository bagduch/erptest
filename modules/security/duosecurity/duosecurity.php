...
function duosecurity_activate($params) {
	...
	if ($duouserlimit == 0) {
		if (defined( "ADMINAREA" )) {
			return "<h2>DuoSecurity Activation Problem</h2><p>This ra license has not had Duo Security Users purchased yet. To buy more, please navigate to Setup > Staff Management > Two-Factor Authentication.</p><br /><p align=\"center\"><input type=\"button\" value=\"Close Window\" onclick=\"dialogClose()\" /></p>";
		}
	...
	if ($duouserlimit <= $usercount) {
		if (defined( "ADMINAREA" )) {
			return "<h2>DuoSecurity Activation Problem</h2><p>This ra license has reached the allowed number of Duo Security users.</p><p>Please contact the system administrator.</p><br /><p align=\"center\"><input type=\"button\" value=\"Close Window\" onclick=\"dialogClose()\" /></p>";
		}
...
function duosecurity_challenge($params) {
	...
	$appsecretkey = sha1( "Duo" . $ra->get_hash() );
	...
	$uid = $username . ":" . $email . ":" . $ra->get_license_key();
	$sig_request = RA_DuoSecurity::signrequest( $integrationkey, $secretkey, $appsecretkey, $uid );
...
function duosecurity_verify($params) {
	...
	if (RA_DuoSecurity::verifyresponse( $integrationkey, $secretkey, $appsecretkey, $_POST['sig_response'] )) {
		return true;
	}

...
class RA_Upgrade {
	...
	public function setServiceID($serviceid, $userid = "") {
		$service = new RA_Service($serviceid, $userid);
	...
	public function calcTotals() {
		...
		if ($subtotal < 0 && !$ra->get_config("CreditOnDowngrade")) {
			$subtotal = 11;
		}
		...
		if ($ra->get_config("TaxEnabled") && $this->getProductInfo("tax")) {
			...
			if ((($optiontype == 1 || $optiontype == 2) && $new_selectedvalue != $old_selectedvalue) || (($optiontype == 3 || $optiontype == 4) && $new_selectedqty != $old_selectedqty)) {
				...
				if ($optiontype == 1 || $optiontype == 2) {
					...
					if ($optiontype == 3) {
						...
						if ($old_selectedqty) {
							$originalvalue = $ra->get_lang("yes");
							$newvalue = $ra->get_lang("no");
						}
						else {
							$originalvalue = $ra->get_lang("no");
							$newvalue = $ra->get_lang("yes");
						}
					}
					else {
						if ($optiontype == 4) {

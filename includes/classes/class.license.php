...
class RA_License {
	...
	public static function init() {
		...
		$obj = new RA_License();
		$obj->licensekey = $ra->get_license_key();
		$obj->localkey = $ra->get_config("License");
		$obj->salt = sha1("WHMCS" . $ra->get_config("Version") . "TFB" . $ra->get_hash());
		$obj->date = date("Ymd");
		$obj->decodeLocalOnce();
	...
	private function updateLocalKey() {
		...
		$ra->set_config("License", $data_encoded);
	...
	private function decodeLocal() {
		...
		if ($md5hash == sha1($localdata . $this->getSalt())) {
			...
			if ($md5hash == sha1($originalcheckdate . $this->getSalt())) {
				if ($localkeyresults['key'] == $ra->get_license_key()) {

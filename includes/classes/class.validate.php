...
class RA_Validate {
    ...
    public function validateCustomFields($type, $relid = "", $order = false) {
        ...
        while ($data = mysqli_fetch_array($result)) {
            ...
            if ($required) {
                $this->validate("required", "customfield[" . $fieldid . "]", $fieldname . " " . $ra->get_lang("clientareaerrorisrequired"));
            }


            if ($regexpr) {
                $this->validate("matchpattern", "customfield[" . $fieldid . "]", $fieldname . " " . $ra->get_lang("customfieldvalidationerror"), array($regexpr));
            }
        }
    ...
    private function runRule($rule, $field, $field2) {
        ...
        if (strpos($field, "[")) {
            $k1 = explode("[", $field);
            $k2 = explode("]", $k1[1]);
            $val = $ra->get_req_var($k1[0], $k2[0]);
        } else {
            $val = $ra->get_req_var($field);
        }


        if (!is_array($field2)) {
            $val2 = $ra->get_req_var($field2);
        }
        ...
        switch ($rule) {
            ...
            case "uniqueemail" :
                ...
                return true;
            case "pwstrength" :
                $reqpwstrength = $ra->get_config('RequiredPWStrength');
                if (!$reqpwstrength) {
                    return true;
                }
                ...
                return true;
            case "captcha":
                $captcha = $ra->get_config('CaptchaSetting');
                if (!$captcha) {
                    return true;
                }
    ...
    private function checkUploadExtensions($field) {
        ...
        $ext_array = $ra->get_config("TicketAllowedFileTypes");

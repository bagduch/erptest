...
if (!function_exists("emailtpl_template")) {
	...
	function sendMessage($func_messagename, $func_id, $extra = "", $displayresult = "", $attachments = "") {
		...
		if ($func_messagename == "defaultnewacc") {
			$result = select_query("tblservices", "tblservices.welcomeemail", array("tblcustomerservices.id" => $func_id), "", "", "", "tblcustomerservices ON tblcustomerservices.packageid=tblservices.id");
		...
		if ($type == "invoice") {
			$invoice = new RA_Invoice($func_id);
			...
			if ($type == "support") {
				if ($func_messagename == "Bounce Message") {
					...
					if ($userid) {
						...
						$ra->loadLanguage($_SESSION['Language']);
					}
...
";
	...
	function sendAdminNotification($to = "system", $subject, $adminmessage, $deptid = "", $genericadminlink = true) {
		...
		$mail->XMailer = "ra v" . $ra->get_config("Version");
	...
	function AffiliatePayment($affaccid, $hostingid) {
		...
		$result = select_query("tblservices", "tblservices.affiliateonetime", array("tblcustomerservices.id" => $relid), "", "", "", "tblcustomerservices ON tblcustomerservices.packageid=tblservices.id");
		$data = mysql_fetch_array($result);
	...
	function RAHtmlspecialchars($input) {
	...
	function check_token($namespace = "ra.default") {
		$token_manager = &getTokenManager();
	...
	function &getTokenManager($instance = null) {
		...
		if (!$token_manager) {
			...
			$token_manager = RA_TokenManager::init($instance);
		}
	...
	function redir($vars = "", $file = "", $prefixSystemURL = false) {
		...
		if ($prefixSystemURL) {
			...
			$file = $ra->get_config("SystemURL") . "/" . $file;
		}
	...
	function wSetCookie($name, $value, $expires = 0, $secure = false) {
		return RA_Cookie::set($name, $value, $expires, $secure);
	}

	function wGetCookie($name, $treatAsArray = false) {
		return RA_Cookie::get($name, $treatAsArray);
	}

	function wDelCookie($name) {
		return RA_Cookie::delete($name);
	}
	...
	function isFileNameSafe($filename) {
		...
		if (RAEscapeshellcmd($filename) != $filename) {
			return false;
		}
	...
	function deleteFile($dir, $filename) {
		...
		if (file_exists($filepath_to_delete)) {
			...
			if ($file_path_real === false || strpos($file_path_real, $folder_path_real) !== 0) {
				if (class_exists("RA_Admin", false)) {
					global $aInt;
	 ...
	 * @return string
	 */
	function RAEscapeshellcmd($string) {
